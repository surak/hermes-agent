...
A custom `base_url` (`http://localhost:11434/v1`, a vLLM, SGLang or router endpoint) receives the resolved effort — `agent.reasoning_effort` or the matching per-model override — as the standard top-level `reasoning_effort` request field, clamped to the values the wire accepts. The ladder is keyed on the model, not the hostname: generic OpenAI-compatible wires accept `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, but a relay fronting Kimi K3 (e.g. a PTJ gateway that maps `reasoning_effort` onto Kimi's `thinking_effort` knob) accepts only `low`, `high`, `max` — `xhigh` is clamped to `max` and `medium` to `high` automatically, so no per-model override is needed for K3 slugs (`k3`, `kimi-k3*`, `Kimi-K3-1M`). The nested `reasoning` object is reserved for endpoints known to accept it (Nous Portal, OpenRouter reasoning-capable models, GitHub Models) because arbitrary servers reject unknown fields with HTTP 400. If your server reads its thinking budget from a different field (Ollama's `think`, vLLM's `chat_template_kwargs`, a router-specific key), set it under the custom provider's [`extra_body`](../integrations/providers.md#named-custom-providers), which is merged into every request routed there.
:::
